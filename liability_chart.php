...
$r=$db->query('select liability.*,people.name from liability join people on people.id=liability.people_id');
$ali=0;
while($s=$r->fetch_assoc()){
    
    
  
    $ss=$db->query("select sum(amount) as total from liability where id =".$s['people_id']);
    $uu=$ss->fetch_assoc();
